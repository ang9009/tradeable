...
data class LoopSongState(val state: SongState, val loopNum: Int);

// Gets the string output to the console given the current state
fun songStateToText(songState: LoopSongState): String {
    return when(songState.state) {
...
// Returns the next state given a state and the user's input
fun nextSongState(currState: LoopSongState, input: String): LoopSongState {
    if(currState.state == SongState.EASY123 && input.equals("done")){
        return LoopSongState(SongState.DONE, currState.loopNum);
    } else if(currState.state == SongState.ABC || currState.state === SongState.START) {
        return LoopSongState(SongState.EASY123, currState.loopNum + 1);
    } else {
        return LoopSongState(SongState.ABC, currState.loopNum);
    }
}

// Determines if current state is terminal state
fun isDone(currState: LoopSongState): Boolean {
    return currState.state === SongState.DONE;
}

// Main function
fun loopSong(): Int {
    val initialState = LoopSongState(SongState.START, 0);
    val finalState = reactConsole(initialState, ::songStateToText, ::nextSongState, ::isDone);
    return finalState.loopNum;
}
